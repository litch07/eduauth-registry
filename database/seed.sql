...
-- -----------------------------------------------------------------------------
-- Students
-- NID hashes are SHA-256 of placeholder NID strings for demo
-- All have DOB: 2002-01-01 (used for certificate verification demos)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Enrollments
-- Various statuses to demo the enrollment management UI
-- -----------------------------------------------------------------------------
...
-- current_sequence = 6 because we are inserting 6 certificates below
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Certificates
-- Serials use the BSC-YY-SEQSEQC checksum format
-- Certificates 1-3: Active & Public (graduated students)
-- Certificate 4:    Private (student can demo toggle-visibility)
-- Certificate 5:    Revoked (demonstrates revocation feature)
-- Certificate 6:    Masters-level
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Access Requests
-- Various statuses to demo the full access request workflow
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Withdrawal Requests
-- Student 2 has requested a withdrawal from their enrollment
-- -----------------------------------------------------------------------------
...
-- Demo: one pending, one approved, one rejected
-- -----------------------------------------------------------------------------
INSERT INTO profile_change_requests (id, student_id, field_name, old_value, new_value, status, reviewed_by, reviewed_at, rejection_note, supporting_documents, created_at, updated_at) VALUES
(1, 1, 'phone',    '+8801700001001', '+8801811112222', 'pending',  NULL, NULL, NULL, NULL, NOW(), NOW()),
(2, 2, 'address',  'Mohammadpur, Dhaka', 'Mirpur-10, Dhaka-1216', 'approved', 1, DATE_SUB(NOW(), INTERVAL 3 DAY), NULL, NULL, DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY)),
(3, 3, 'phone',    '+8801700001003', '+8801922334455', 'rejected', 1, DATE_SUB(NOW(), INTERVAL 1 DAY), 'The provided phone number appears to be invalid. Please verify and resubmit.', NULL, DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY));

-- -----------------------------------------------------------------------------
-- Verification Logs
-- Demonstrates the verifier dashboard's history and analytics
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Activity Logs
-- Populates the admin analytics and user activity tabs
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
INSERT INTO user_settings (user_id, default_certificate_visibility, notify_access_request, notify_certificate_issued, notify_access_approved, notify_access_rejected, notify_profile_change, created_at, updated_at) VALUES
(2,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(3,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(4,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(5,  'private', 1, 1, 1, 1, 1, NOW(), NOW()),
(6,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(7,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(8,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(9,  'public',  1, 1, 1, 1, 1, NOW(), NOW()),
(10, 'public',  1, 1, 1, 1, 1, NOW(), NOW());
